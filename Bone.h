...
private:
    ...
    glm::vec3 GetGLMVec(const aiVector3D& vec) { return glm::vec3(vec.x, vec.y, vec.z); }


    //Helper to transform assimp quaternion to GLM quaternion
    inline glm::quat GetGLMQuat(const aiQuaternion& orientation){ return glm::quat(orientation.w, orientation.x, orientation.y, orientation.z);  }
};
